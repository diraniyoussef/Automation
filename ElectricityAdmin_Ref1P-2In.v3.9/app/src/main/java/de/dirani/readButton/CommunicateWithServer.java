...
class CommunicateWithServer {
    ...
    private char[] includedPinIndexAsChar;
    ...
    CommunicateWithServer(Toasting toasting, Activity act, Context applicationContextParam, SocketConnection parentSocketConnection_arg,
                      String panel_index_arg, String panel_name_arg, int NUMBER_OF_POINTS_arg, char[] includedPinIndexAsChar_arg,
                      Switch[] userSwitch_arg, TextView[] textView_arg, boolean localNotInternet, String owner_part, String mob_part, String mob_Id) {
        ...
        NUMBER_OF_POINTS = NUMBER_OF_POINTS_arg;
        includedPinIndexAsChar = includedPinIndexAsChar_arg;
        userSwitch = userSwitch_arg;
        textView = textView_arg;
        ...
        public void run() {
            ...
            try {
                if( checkMessageValidity() ) { //checkMessageValidity() is about owner, mod, mob.
                    ...
                    int dataIndex = incomingMessage.lastIndexOf(":") + 1; //last index is important...
                    ...
                    if (dataIndex != -1) {
                        ...
                        for (int i = 0; i < NUMBER_OF_POINTS; i++) {
                            textToSearch = "O" + String.valueOf(includedPinIndexAsChar[i]);
                            ...
                            if (specificDataIndex != -1) {
                                ...
                                if (outputState.equals("T")) {
                                    activity.runOnUiThread(new Runnable() {
                                        public void run() {
                                            if( userSwitch != null ) //In case it was the Reflector panel, userSwitch would be null
                                                userSwitch[viewIndex].setChecked(true);
                                            textView[viewIndex].setText("ON");
                                            textView[viewIndex].setBackgroundColor(android.R.drawable.edit_text);
                                            Log.i("Youssef Communi...java", "something did change to True at output " +
                                                    includedPinIndexAsChar[viewIndex] +
                                                    ". For panel index " + panel_index +
                                                    " on port " + parentSocketConnection.selectedServerConfig.getPortFromIndex(parentSocketConnection.active_client_index) );
                                        }
                                    });
                                    ...
                                    if (outputState.equals("F")) {
                                        activity.runOnUiThread(new Runnable() {
                                            public void run() {
                                                if( userSwitch!= null )
                                                    userSwitch[viewIndex].setChecked(false);
                                                textView[viewIndex].setText("OFF");
                                                textView[viewIndex].setBackgroundColor(android.R.drawable.edit_text);
                                        ...
                                        Log.i("Youssef Communi...java", "Couldn't get information correctly from module after not " +
                                                "being false, for panel index " + panel_index +
                                                " on port " + parentSocketConnection.selectedServerConfig.getPortFromIndex(parentSocketConnection.active_client_index));
                                    }
                                }
                            } else {
                                //didn't find O so it has to be ACK
                                textToSearch = "ACK";
                                specificDataIndex = incomingMessage.indexOf(textToSearch, dataIndex);
                                if (specificDataIndex != -1) {
                                    ...
                                    //Generic.toasting.toast("Message received from panel" +
                                      //      panel_name + ".", Toast.LENGTH_SHORT, false);
                                    //Generic.toasting.toast("Message received from module.", Toast.LENGTH_SHORT, silentToastAfterNewSocket);
                                    //silentToastAfterNewSocket = false;
                                    receivedResponse = true;
                                    ...
                                    //Generic.toast(this_is_not_UI_thread, "Couldn't get information correctly from module.", Toast.LENGTH_SHORT, silentToast);
                                    Log.i("Youssef Communi...java", "The message is neither ACK nor a report like O1T..." +
